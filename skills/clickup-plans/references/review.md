...
Collect **unchecked** items — `- [ ]` — from `Feedback` on the parent and `Review Feedback`
on the child. Both pages in one pass: the PM's product objection and a developer's data-model
objection often need the same edit, and processing them separately produces two conflicting
rewrites.
...
| **Question** | Answer it in the spec if the answer is knowable; otherwise move it to `Open Questions` and say who needs to answer |
| **Out of scope** | Propose a `Bewusste Nicht-Ziele` row with the reasoning — do not just decline it in chat, or the same objection returns next month |
...
```markdown
- [x] [Anna] Abschnitt 3: Drip-Intervall sollte pro Kurs einstellbar sein → Datenmodell auf
      CourseEntry-Ebene erweitert, UI bleibt kapitelweise (§2.1)
```

The resolution is what makes this idempotent — re-running never re-processes the item, and
Anna can see her point landed without asking anyone.
...
| Promoting to 🟢 with items still open | Someone implements a spec that is still under discussion |
